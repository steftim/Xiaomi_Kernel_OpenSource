...
info_retry:
	...
	case TP_VENDOR_DJN:
		sprintf(tp_info_buf, "[Vendor]dijing,[FW]0x%02x,[IC]nt36525b\n", ts->fw_ver);
		update_lct_tp_info(tp_info_buf, NULL);
		break;

	case TP_VENDOR_HLT:
		sprintf(tp_info_buf, "[Vendor]helitai,[FW]0x%02x,[IC]nt36525b\n", ts->fw_ver);
		update_lct_tp_info(tp_info_buf, NULL);
		break;
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	switch(ts->touch_vendor_id) {
	case TP_VENDOR_DJN:
		memcpy(ts->boot_update_firmware_name, BOOT_UPDATE_DJN_FIRMWARE_NAME, sizeof(BOOT_UPDATE_DJN_FIRMWARE_NAME));
		memcpy(ts->mp_update_firmware_name, MP_UPDATE_DJN_FIRMWARE_NAME, sizeof(MP_UPDATE_DJN_FIRMWARE_NAME));
		break;

	case TP_VENDOR_HLT:
		memcpy(ts->boot_update_firmware_name, BOOT_UPDATE_HLT_FIRMWARE_NAME, sizeof(BOOT_UPDATE_HLT_FIRMWARE_NAME));
		memcpy(ts->mp_update_firmware_name, MP_UPDATE_HLT_FIRMWARE_NAME, sizeof(MP_UPDATE_HLT_FIRMWARE_NAME));
		break;
...
static int32_t __init nvt_driver_init(void)
{
	...
	if (IS_ERR_OR_NULL(mtkfb_lcm_name)){
		...
		if (strcmp(mtkfb_lcm_name,"nt36525b_vdo_hdp_boe_dijing_drv") == 0) {
			touch_vendor_id = TP_VENDOR_DJN;
			NVT_LOG("TP info: [Vendor]DIJING [IC]nt36525b\n");
		} else if (strcmp(mtkfb_lcm_name,"nt36525b_vdo_hdp_boe_helitai_drv") == 0) {
			touch_vendor_id = TP_VENDOR_HLT;
			NVT_LOG("TP info: [Vendor]HELITAI [IC]nt36525b\n");
